...
local BASE_C0 = {
    RootJoint = _humanoidRootPart.RootJoint.C0;
    Neck = _torso.Neck.C0;
    HipR = _torso["Right Hip"].C0;
    HipL = _torso["Left Hip"].C0;
}

local RANGE_OF_MOTION = 45
local RANGE_OF_MOTION_TORSO = 50-RANGE_OF_MOTION
...
function calculateC0(dt, humanoidRootPart, humanoid, torso)
    local direction = humanoidRootPart.CFrame:VectorToObjectSpace(humanoidRootPart.AssemblyLinearVelocity)
    ...
    local newHipR = BASE_C0.HipR * CFrame.new(-newXZ, 0, -math.abs(newXZ) + math.abs(-newXZ)) * CFrame.Angles(0, -newX, 0)
    local newHipL = BASE_C0.HipL * CFrame.new(-newXZ, 0, -math.abs(newXZ) + math.abs(-newXZ)) * CFrame.Angles(0, -newX, 0)
